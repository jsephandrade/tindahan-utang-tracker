...
const UtangManagement = () => {
  ...
  return <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        <Button variant={filterStatus === 'all' ? 'default' : 'outline'} onClick={() => setFilterStatus('all')} className={`flex-shrink-0 min-w-0 px-3 py-2 ${filterStatus === 'all' ? 'bg-orange-600 hover:bg-orange-700' : ''}`}>
          <span className="whitespace-nowrap">All ({consolidatedRecords.length})</span>
        </Button>
        <Button variant={filterStatus === 'unpaid' ? 'default' : 'outline'} onClick={() => setFilterStatus('unpaid')} className={`flex-shrink-0 min-w-0 px-3 py-2 ${filterStatus === 'unpaid' ? 'bg-red-600 hover:bg-red-700' : ''}`}>
          <span className="whitespace-nowrap">Unpaid ({consolidatedRecords.filter(r => r.status === 'unpaid').length})</span>
        </Button>
        <Button variant={filterStatus === 'partial' ? 'default' : 'outline'} onClick={() => setFilterStatus('partial')} className={`flex-shrink-0 min-w-0 px-3 py-2 ${filterStatus === 'partial' ? 'bg-yellow-600 hover:bg-yellow-700' : ''}`}>
          <span className="whitespace-nowrap">Partial ({consolidatedRecords.filter(r => r.status === 'partial').length})</span>
        </Button>
        <Button variant={filterStatus === 'paid' ? 'default' : 'outline'} onClick={() => setFilterStatus('paid')} className={`flex-shrink-0 min-w-0 px-3 py-2 ${filterStatus === 'paid' ? 'bg-green-600 hover:bg-green-700' : ''}`}>
                ...
                <CardContent className="p-4 sm:p-6 space-y-4">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                    <div className="flex-1 space-y-3">
                      ...
                      {allItems.length > 0 && <div className="bg-gray-50 p-3 rounded-lg border">
                          ...
                          <div className="space-y-1 font-mono text-sm">
                            ...
                            {allItems.map((item, index) => <div key={`${item.transactionId}-${index}`} className="flex justify-between items-start">
                    ...
                    <div className="text-right space-y-2 flex-shrink-0">
                      ...
                      {consolidated.totalPaid > 0 && <div>
                          <p className="text-sm text-muted-foreground">Paid</p>
                          <p className="text-lg font-semibold text-green-600">P{consolidated.totalPaid.toFixed(2)}</p>
                        </div>}
                      
                      {consolidated.remainingBalance > 0 && <div>
                          <p className="text-sm text-muted-foreground">Balance</p>
                          <p className="text-lg font-semibold text-red-600">P{consolidated.remainingBalance.toFixed(2)}</p>
                        </div>}
                      
                      {consolidated.status !== 'paid' && <Button onClick={() => openPaymentDialog(consolidated)} className="bg-orange-600 hover:bg-orange-700 w-full sm:w-auto" size="sm">
                  ...
                  {/* Payment History */}
                  {allPayments.length > 0 && <div className="border-t pt-4 space-y-2">
                      <h4 className="font-semibold text-sm flex items-center gap-2">
                        <DollarSign className="h-4 w-4" />
                        Payment History
                      </h4>
                      <div className="space-y-2">
                        {allPayments.sort((a, b) => b.date.getTime() - a.date.getTime()).map(payment => <div key={payment.id} className="flex justify-between items-center text-sm bg-green-50 p-2 rounded">
                            <div>
        ...
        <DialogContent>
          ...
          {selectedCustomer && <div className="space-y-4">
              ...
              <div>
                ...
                <Input id="payment-amount" type="number" value={paymentAmount} onChange={e => setPaymentAmount(Number(e.target.value))} placeholder="0.00" max={selectedCustomer.remainingBalance} />
              </div>
              
              <div>
                <Label htmlFor="payment-note">Note (Optional)</Label>
                <Input id="payment-note" value={paymentNote} onChange={e => setPaymentNote(e.target.value)} placeholder="Add a note for this payment" />
              </div>
